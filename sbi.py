...
with sync_playwright() as p:
    ...
    page.locator("//input[@name='user_id']").fill(os.environ.get("SBI_ACCOUNT"))
    page.locator("//div[@id='password_input']/input").fill(
    ...
    page.wait_for_load_state("networkidle")
    total_count = int(
        page.locator("//div[contains(@class,'control__counter')]/span").inner_text()
    )
    items = page.locator(
        "//mat-accordion[contains(@class,'items')]/mat-expansion-panel"
    )
    ...
    while items.count() < total_count and items.count() < 100:
        ...
        page.wait_for_load_state("networkidle")
        items = page.locator(
            "//mat-accordion[contains(@class,'item')]/mat-expansion-panel"
        )
        print(f"Found {items.count()=} items")
    print(f"Found {items.count()} items")
    for idx in range(0, items.count()):
        ...
        if (
            not FETCH_ALL
            and item.locator(
                "//mat-expansion-panel-header[not(contains(@class,'-read'))]"
            ).count()
            == 0
        ):
            continue
        ...
        with page.expect_download() as pdf_new_tab:
            ...
            pdf_file = Path(PDF_DIR) / Path(
                f"{''.join(year_month)}/{date.replace('/','-')} {filename}.pdf"
            )
